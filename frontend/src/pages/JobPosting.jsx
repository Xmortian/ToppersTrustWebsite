...
import { supabase } from '../supabase.js'; // Import if needed

// Icons
import { IoClose } from "react-icons/io5";
import { FaTimesCircle } from "react-icons/fa"; // For removing selected subjects
...
// Dhaka Thanas List (Unchanged)
const dhakaThanas = [ "", "Adabor", "Badda", "Bangshal", "Bimanbandar", "Chakbazar", "Dakshinkhan", "Darus Salam", "Demra", "Dhanmondi", "Gendaria", "Gulshan", "Hazaribagh", "Jatrabari", "Kadamtali", "Kafrul", "Kalabagan", "Kamrangirchar", "Khilgaon", "Khilkhet", "Kotwali", "Lalbagh", "Mirpur", "Mohammadpur", "Motijheel", "New Market", "Pallabi", "Paltan", "Ramna", "Rampura", "Sabujbagh", "Shah Ali", "Shahbagh", "Sher-e-Bangla Nagar", "Shyampur", "Sutrapur", "Tejgaon", "Tejgaon Industrial Area", "Turag", "Uttar Khan", "Uttara East", "Uttara West", "Vatara", "Wari", "Ashulia", "Dhamrai", "Dohar", "Keraniganj", "Nawabganj", "Savar", "Other" ];

// Location Data Structure (Unchanged)
const locationData = {
    "Dhaka": dhakaThanas,
    "Chittagong": ["", "Agrabad", "Nasirabad", "Pahartali", "Kotwali", "Other"],
    "Khulna": ["", "Khulna Sadar", "Sonadanga", "Daulatpur", "Other"],
    // Add other cities and their locations
};
...
const JobPosting = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        studentGender: '', // Default to empty, force selection between Male/Female
        city: 'Dhaka',
        category: 'English Medium',
        subjects: [], // Array for selected subjects
        location: '',
        address: '',
    ...
    const [subjectToAdd, setSubjectToAdd] = useState(""); // State for the subject dropdown
    const [subjectError, setSubjectError] = useState("");

    // Update locations when city changes (Unchanged)
    useEffect(() => {
    ...
    // Updated for only Male/Female options
    const handleStudentGenderChange = (value) => {
        setFormData(prev => ({ ...prev, studentGender: value }));
        setMessage({ type: '', text: '' });
    };

    const handleTutorGenderChange = (value) => {
        setFormData(prev => ({ ...prev, tutorGenderPref: value }));
        setMessage({ type: '', text: '' });
    };

    // Handle adding a subject from the dropdown
    const addSubject = () => {
        setSubjectError(""); // Clear previous error
        if (subjectToAdd && !formData.subjects.includes(subjectToAdd)) {
            if (formData.subjects.length < 5) {
                ...
                setSubjectToAdd(""); // Reset dropdown after adding
            } else {
     ...
     // Handle removing a selected subject (Unchanged)
    const removeSubject = (subjectToRemove) => {
        setFormData(prev => ({
            ...prev,
            subjects: prev.subjects.filter(subject => subject !== subjectToRemove)
        }));
        setSubjectError(""); // Clear error on removal
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.subjects.length === 0) {
         ...
         if (!formData.studentGender) { // Check if student gender is selected
        ...
        // --- TODO: Implement Supabase Insert Logic ---
        console.log("Submitting Job Posting Data:", formData);
        // Example:
        ...
        setMessage({ type: 'success', text: 'Job posted successfully! (Placeholder)' });
        setIsLoading(false);
    };

    // Helper component for Gender Toggle Buttons (Updated to accept options)
            ...
            <div className="flex items-center gap-0 rounded-md border border-gray-300 overflow-hidden text-xs sm:text-sm">
                {options.map(option => (
                    <button
                        key={option}
                        type="button"
                        onClick={() => onChange(option)} // Pass the value directly
    ...
    // Helper component for Select Dropdowns (Unchanged)
    const SelectInput = ({ name, label, value, onChange, options, required = false }) => (
         <div>
            ...
            >
                {options.map((option, index) => ( <option key={index} value={option}> {option || `-- Select ${label} --`} </option> ))}
            </select>
        </div>
    );

    // Helper component for Text Inputs (Unchanged)
     const TextInput = ({ name, label, value, onChange, placeholder = "", type = "text", required = false }) => (
         <div>
            <label htmlFor={name} className="block text-xs font-medium text-gray-600 mb-1">{label}</label>
            ...
            <form onSubmit={handleSubmit} className="w-full max-w-4xl bg-white p-6 sm:p-8 rounded-xl shadow-lg">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
                    ...
                    <div className="space-y-5">
                        ...
                        {/* Tutor Gender Preference - Still includes Any */}
                        ...
                        <SelectInput name="tuitionType" label="Tuition Type" value={formData.tuitionType} onChange={handleInputChange} options={["", "Home Tutoring", "Online Tutoring"]} required />
                        {/* Student Gender - Only Male/Female */}
                    ...
                    {/* Column 2 */}
                     <div className="space-y-5">
                        ...
                        <SelectInput name="location" label="Select Location (Thana)" value={formData.location} onChange={handleInputChange} options={availableLocations} required />
                        ...
                        {/* Improved Subjects Multi-Select */}
                        <div>
                             ...
                             {formData.subjects.length > 0 && (
                                 <div className="mt-2 flex flex-wrap gap-1.5 border p-2 rounded-md min-h-[40px]">
                                     {formData.subjects.map((subject, index) => (
                                         <span key={index} className="flex items-center bg-purple-100 text-purple-800 text-xs font-medium pl-2 pr-1 py-0.5 rounded-full h-fit">
                                             {subject}
                                             <button
                                                 type="button"
                    ...
                    {/* Column 3 */}
                     <div className="space-y-5">
                         <TextInput name="address" label="Full Address" value={formData.address} onChange={handleInputChange} placeholder="E.g: Flat:3E, Tropical Homes, House:12, Road..." required />
                         ...
                         <TextInput name="startingDate" label="Starting Date" value={formData.startingDate} onChange={handleInputChange} type="date" required />
                         <SelectInput name="paymentType" label="Payment" value={formData.paymentType} onChange={handleInputChange} options={paymentOptions} required />
                         <div>
                ...
                {/* Message Display and Submit Button */}
